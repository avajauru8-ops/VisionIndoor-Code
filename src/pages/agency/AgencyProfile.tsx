...
export default function AgencyProfile() {
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div>
        <h2 className="text-2xl font-extrabold text-[#0b462c] tracking-tight">Dados da AgÃªncia</h2>
        <p className="text-xs text-[#8b9aa5] font-medium mt-1">Configure informaÃ§Ãµes essenciais da sua agÃªncia e os dados para contratos.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-[#e8edf2] rounded-[24px] p-8 space-y-6 shadow-sm">
         ...
         <div className="flex items-center gap-6 pb-6 border-b border-[#e8edf2]">
            ...
            <div>
               <h3 className="text-[#0b462c] text-xs font-bold uppercase tracking-wider mb-1">Logomarca da AgÃªncia</h3>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-2">Nome da AgÃªncia</label>
            ...
            <div>
              <label className="block text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-2">Cidades de AtuaÃ§Ã£o</label>
              <input type="text" name="cidades_atuacao" value={profile.cidades_atuacao || ''} onChange={handleChange} className="w-full bg-[#f4f6f8] border border-zinc-200 rounded-xl px-4 py-2.5 text-zinc-800 text-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none transition-all" placeholder="Ex: SÃ£o Paulo, Campinas" />
            </div>
         </div>
         ...
         <div className="pt-6 flex justify-end border-t border-[#e8edf2] mt-6">
            ...
            >
              {saving ? 'SALVANDO...' : 'SALVAR INFORMAÃ‡Ã•ES'}
            </button>
         </div>
      </form>
    </div>
